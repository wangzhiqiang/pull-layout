...
import com.dianjineng.android.lib.pulllayout.pull.PullDefault;
...
public class PullLayout extends FrameLayout {
    ...
    private StatePull mState = StatePull.INIT;
    ...
    // 释放刷新的距离
    private int mRefreshDist = 200;
    // 释放加载的距离
    private int mLoadMoreDist = 200;
    ...
    private PullStateListener mPullableStateListener = new PullDefault();
    // 刷新回调接口
    ...
    Handler updateHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (!isTouch) {
                ...
                if (mState == StatePull.REFRESHING && pullDownY <= mRefreshDist) {
                    pullDownY = mRefreshDist;
                    timer.cancel();
                } else if (mState == StatePull.LOADING && -pullUpY <= mLoadMoreDist) {
...
//                mImageRefreshArrow.clearAnimation();
                ...
                if (mState != StatePull.REFRESHING && mState != StatePull.LOADING) {
                    changeState(StatePull.INIT);
                }
                timer.cancel();
                requestLayout();
            }
...
//                mImageLoadArrow.clearAnimation();
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        if (null != attrs) {
            ...
            int dist = a.getDimensionPixelSize(R.styleable.PullLayout_dist, 100);

            mRefreshDist  = a.getDimensionPixelSize(R.styleable.PullLayout_distRefresh,   dist);
            mLoadMoreDist = a.getDimensionPixelSize(R.styleable.PullLayout_distLoadMore,  dist);
            a.recycle();
        }
    ...
    private void changeState(StatePull to) {
        mState = to;
        switch (mState) {
            case INIT:
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mEvents == 0) {
                    if (pullDownY > 0
                            ...
                            && mCanPullDown && mState != StatePull.LOADING)) {
                        // 可以下拉，正在加载时不能下拉
                        // 对实际滑动距离做缩小，造成用力拉的感觉
                        ...
                        if (mState == StatePull.REFRESHING) {
                            // 正在刷新的时候触摸移动
                            isTouch = true;
                        }
                    } else if (pullUpY < 0
                            || (mPullHandler.canPullUp() && mCanPullUp && mState != StatePull.REFRESHING)) {
                        // 可以上拉，正在刷新时不能上拉
                ...
                if (pullDownY > 0) {
                    if (pullDownY <= mRefreshDist
                            && (mState == StatePull.RELEASE_TO_REFRESH || mState == StatePull.DONE)) {
                        // 如果下拉距离没达到刷新的距离且当前状态是释放刷新，改变状态为下拉刷新
                        changeState(StatePull.INIT);
                    }

                    mPullableStateListener.onPull(pullDownY,mRefreshDist);
                    if (pullDownY >= mRefreshDist && mState == StatePull.INIT) {
                        // 如果下拉距离达到刷新的距离且当前状态是初始状态刷新，改变状态为释放刷新
                        changeState(StatePull.RELEASE_TO_REFRESH);
                    }
                } else if (pullUpY < 0) {
                    mPullableStateListener.onPull(pullUpY,mLoadMoreDist);
                    // 下面是判断上拉加载的，同上，注意pullUpY是负值
                    if (-pullUpY <= mLoadMoreDist
                            && (mState == StatePull.RELEASE_TO_LOAD || mState == StatePull.DONE)) {
                        changeState(StatePull.INIT);
                    }
                    // 上拉操作
                    if (-pullUpY >= mLoadMoreDist && mState == StatePull.INIT) {
                        changeState(StatePull.RELEASE_TO_LOAD);
                    }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mState == StatePull.RELEASE_TO_REFRESH) {
                    changeState(StatePull.REFRESHING);
                    // 刷新操作
                    if (mListener != null)
                        mListener.onRefresh(this);
                } else if (mState == StatePull.RELEASE_TO_LOAD) {
                    changeState(StatePull.LOADING);
                    // 加载操作
                    if (mListener != null)
...
//    }
    ...
    private interface FinishListener {
        void refreshFinish(StateFinish state);

        void loadFinish(StateFinish state);
    }
    ...
    public interface PullStateListener extends FinishListener, ViewInitListener {
        ...
        /**
         *
         * @param y refresh或loadMore的滑动距离,refresh为正 loadMore为负
         * @param dist 释放后执行操作的距离
         */
        void onPull(float y,int dist);
